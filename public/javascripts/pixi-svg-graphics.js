...
/***/ function(module, exports, __webpack_require__) {
	...
	SVGGraphics.prototype.drawNode = function (node) {
	  ...
	  var capitalizedTagName = tagName.charAt(0).toUpperCase() + tagName.slice(1)
		if(capitalizedTagName == 'Style'){
			this.style = new Array();
			var html = node.innerHTML
			var pairs = html.split('}')
	    for (var j = 0, len = pairs.length; j < len; j++) {
				pairs[j] = pairs[j].trim()
	      if ((!pairs[j]) || (pairs[j]=="") || (pairs[j]==" ")) {
					pairs.splice(j)
					continue
	      }
				var _pairs = pairs[j].split('{')
				for(var k = 0,kLen = _pairs.length;k<kLen;k++){
					_pairs[k] = _pairs[k].trim()
					if ((!_pairs[k]) || (_pairs[k]=="") || (_pairs[k]==" ")) {
						_pairs.splice(k)
						continue
		      }
				}
				var _tmp = new Object()
				_tmp[_pairs[0]] = _pairs[1]
				this.style.push(_tmp)
	    }
			return
		}
	 ...
	 * @param  {SVGLinearGradientElement} node
	 */
	SVGGraphics.prototype.drawLinearGradientNode = function (node) {
	  console.log('渐变无法显示');
		// var html = node.innerHTML
		// console.log(html);
		// console.log(this._graphics);
		// var grd = this._graphics.createLinearGradient(27,55,27,0.5)
		// grd.addColorStop(0,'#0E213F')
		// grd.addColorStop(1,'#122E57')
	}
	...
	SVGGraphics.prototype.applySvgAttributes = function (node) {
	  ...
	  var style = node.getAttribute('style')
		//如果是有style标签的，获取style标签的内容，再把相应的内容设置为当前node
		if(this.style){
			var _class = '.' + node.getAttribute('class')
			for(var _i in this.style){
				if(typeof this.style[_i][_class] == "string"){
					style = this.style[_i][_class]
				}
			}
		}
